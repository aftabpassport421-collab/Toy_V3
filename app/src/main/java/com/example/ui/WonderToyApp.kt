  ...
  Scaffold(
    topBar = {
      WonderToyTopBar(
        ...
        onLocationClick = { currentTab = BottomTab.STORES }
      )
    },
    bottomBar = {
      NavigationBar(
    ...
    Box(
      ...
      when (currentTab) {
        ...
        BottomTab.STORES -> {
          StoresScreen(
            ...
            onSelectStore = { onSetSelectedStore(it) }
          )
        }
      }
      ...
      // Dialog 2: Checkout Bottom Sheet (Parental Lock Removed, direct checkout)
      if (uiState.isCheckoutVisible) {
        CheckoutDialog(
          ...
          onStoreChange = { onSetSelectedStore(it) },
